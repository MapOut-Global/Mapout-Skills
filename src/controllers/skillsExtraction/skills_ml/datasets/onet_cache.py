...
from skills_ml.datasets.onet_source import OnetToMemoryDownloader,OnetToDiskDownloader
from skills_ml.storage import InMemoryStore,FSStore
...
class OnetSiteCache(object):
    ...
    def reader(self, filename,get_path=False):
        """
        Ensures that the given ONET data file is present, either by
        using a cached copy or downloading from S3
        ...
        if get_path:
            return csv.DictReader(io.StringIO(contents.decode('utf-8')), delimiter='\t'),os.path.join(self.storage.path, filename)
