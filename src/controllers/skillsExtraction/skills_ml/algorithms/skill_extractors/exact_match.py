...
class ExactMatchSkillExtractor(ListBasedSkillExtractor):
    ...
    def candidate_skills_raw(self, raw_text: str) -> CandidateSkillYielder:
        ...
        sentences = self.nlp.sentence_tokenize(self.nlp.clean_str(raw_text), include_spans=True)
        for sent in sentences:
            matches = self.lookup_regex.finditer(sent.text)
            for match in matches: 
                # logging.info('Yielding exact match %s in string %s', match, sent.text)
                yield CandidateSkill(
                    skill_name=match[0].lower(),
                    matched_skill_identifier=self.id_lookup[match[0].lower()],
                    confidence=100,
                    context=sent.text,
                    start_index=sent.start_index + match.start(),
                    document_id='',
                    document_type='',
                    source_object='',
